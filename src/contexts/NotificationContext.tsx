    ...
    const refreshNotifications = async () => {
      try {
        ...
        rfqs.forEach((rfq) => {
          ...
          pushNotification({
            title: 'RFQ baru',
            message: `RFQ ${rfq.rfq_number} dari ${rfq.company_name || 'client'} dibuat.`,
          });
        });
            ...
            if (!isCreator) {
              if (isManager && quotation.status === 'waiting') {
                pushNotification({
                  ...
                  message: `Quotation ${quotation.quotation_number} menunggu approval.`,
                });
              } else {
                pushNotification({
                  title: 'Quotation baru',
                  message: `Quotation ${quotation.quotation_number} dibuat.`,
                });
              }
            }
            return;
          ...
          if (previousQuotation.status && previousQuotation.status !== quotation.status) {
            ...
            if (notifyForStatus && (isCreator || isManager)) {
              ...
              pushNotification({
                ...
                message: `Quotation ${quotation.quotation_number} ${statusLabel}.`,
              });
            }
          }
        });
          ...
          if (!previousOrder) {
            if (!isCreator) {
              pushNotification({
                ...
                message: `Sales order ${order.order_number} dibuat.`,
              });
            }
            return;
          }
          if (previousOrder.status && previousOrder.status !== order.status) {
            if (order.status === 'waiting approval' && isManager) {
              pushNotification({
                ...
                message: `Sales order ${order.order_number} menunggu approval.`,
              });
            }
          }
        });
        ...
        deliveryOrders.forEach((delivery) => {
          ...
          pushNotification({
            ...
            message: `Delivery order ${delivery.delivery_number} dibuat.`,
          });
        });
        ...
        invoices.forEach((invoice) => {
          ...
          if (invoice.status === 'paid') {
            ...
            pushNotification({
              title: 'Invoice paid',
              message: `Invoice ${invoice.invoice_number} telah dibayar.`,
            });
          }
        });
