...
export default function Clients() {
  ...
  const addShipAddress = () => {
    setFormData((prev) => {
      if (prev.ship_addresses.length >= MAX_SHIP_ADDRESSES) return prev;
      return { ...prev, ship_addresses: [...prev.ship_addresses, ''] };
    });
  };
  ...
  const removeShipAddress = (index: number) => {
    setFormData((prev) => ({
      ...prev,
      ship_addresses: prev.ship_addresses.filter((_, idx) => idx !== index),
    }));
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: ClientFormData = {
      ...
      email: emailValue,
      phone: phoneValue,
      ship_addresses: shipAddresses.slice(0, MAX_SHIP_ADDRESSES),
              ...
              <div>
                  ...
                  <label className="flex items-center gap-2 text-sm text-gray-600">
                    <input
                      ...
                      onChange={(event) => {
                        ...
                        setSameAsAddress(isSame);
                        if (!isSame && formData.ship_addresses.length === 0) {
                          setFormData((prev) => ({ ...prev, ship_addresses: [''] }));
                        }
                      }}
                    />
                    Same as address
                ...
                {sameAsAddress ? (
                  ...
                  <div className="mt-2 space-y-2">
                    {formData.ship_addresses.map((address, index) => (
                      <div key={`ship-${index}`} className="flex items-center gap-2">
                        <input
                          type="text"
                          value={address}
                        ...
                        />
                        {formData.ship_addresses.length > 1 && (
                          <button
                            type="button"
                    ...
                    <div>
                      <button
                        ...
                        disabled={formData.ship_addresses.length >= MAX_SHIP_ADDRESSES}
                        className="text-sm text-blue-600 hover:text-blue-700 disabled:text-gray-400"
                      >
                        + Add shipping address
                      </button>
                      <p className="text-xs text-gray-500 mt-1">
                        Up to {MAX_SHIP_ADDRESSES} shipping addresses.
                      </p>
                    </div>
                  </div>
                )}
