...
type ChatMessage = {
  id: string;
  message: string;
  sender: { id: number | string; role: string; name?: string | null; photo_url?: string | null };
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          ref={widgetRef}
          className="fixed bottom-24 right-6 z-50 flex h-[32rem] w-[22rem] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900"
        >
          <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-slate-800">
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">Forum Chat</p>
              ...
              groupedMessages.map((group) => (
                <div key={group.label} className="space-y-3">
                  ...
                  {group.messages.map((msg) => {
                    ...
                    const avatar = msg.sender.photo_url ? `${apiRoot}${msg.sender.photo_url}` : null;
                    return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            {avatar ? (
                              <img
                                src={avatar}
                                alt={msg.sender.name || msg.sender.role}
                                className="h-6 w-6 rounded-full object-cover"
                              />
                            ) : (
                              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-white/70 text-[10px] font-semibold text-blue-600">
                                {(msg.sender.name || msg.sender.role || 'U')
                                  .split(' ')
                                  .map((part) => part[0])
                                  .slice(0, 2)
                                  .join('')
                            ...
                            <div className="mt-2">
                              {msg.attachment.type?.startsWith('audio') ? (
                                <audio controls className="w-full">
                                  <source src={`${apiRoot}${msg.attachment.url}`} type={msg.attachment.type || undefined} />
                                </audio>
                              ) : (
                                <a
                                  href={`${apiRoot}${msg.attachment.url}`}
